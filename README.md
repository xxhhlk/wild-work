...
> - 商汤小浣熊与 Loomy 是**导入型**渠道：面板按钮为「从本机客户端导入」，不走浏览器登录。
>   （小浣熊官方的登录本身是「网页授权 + 深链回调 `office-raccoon://auth/callback`」，
>   但回调地址由服务端硬编码、不支持 `redirect_uri`，第三方无法复现 —— 详见 `docs/raccoon渠道接入备忘.md` §11。）
>   **导入后请退出对应客户端** —— 小浣熊的 refresh_token 是单会话的，两边同时刷新会互相作废。
