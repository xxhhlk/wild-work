...
> 多渠道账号聚合桌面工具——把 WorkBuddy(CodeBuddy) 国内版/国际版、TraeWork、QoderCN、QoderCOM（国际版）、千问办公、商汤小浣熊、Loomy（讯飞）、MonkeyCode 平台托管模型的多个账号聚合成一个 OpenAI 兼容 API，并额外提供**无需账号**的 OpenCodeZen 匿名免费通道，双击启动，浏览器管理。
...
- **十一渠道聚合**：WorkBuddyCN(CodeBuddy) + WorkBuddyAI（国际版） + TraeWork + TraeCode + QoderCN + QoderCOM（国际版） + 千问办公 + 商汤小浣熊 + Loomy（讯飞） + MonkeyCode（`monkeycode/*`，平台托管模型） + OpenCodeZen（`oczen/*`，匿名免费、无需账号）。
  商汤小浣熊支持两条路：面板登录按钮走**浏览器授权**（登录期间临时把 `office-raccoon://` 协议回调指向本工具以接住授权码，结束立即恢复注册表），弹窗里的次按钮「从客户端导入」则复用本机客户端凭据；Loomy / MonkeyCode 为**导入型**（凭据不由本工具登录产生，从**本机已登录的官方客户端**读取）。模型前缀路由，粘性路由优先复用账号以提升会话缓存利用率，临期积分优先消耗；旧 Qoder（`qoder/*`）路由保留但已从界面下线
...
| TraeCode | `traecode/*` | — | ✓（共用 TraeWork） | ✓ | —（别名渠道） | refresh（共用 TraeWork） |
| QoderCN | `qodercn/*` | ✓ | ✓ | ✓ | 10:00–12:00 窗口重试 | refresh（≈30 天） |
| QoderCOM | `qodercom/*` | ✓ | ✓ | ✓ | 10:00–12:00 窗口重试 | refresh（≈30 天） |
...
| MonkeyCode | `monkeycode/*` | — | ✓（积分 + 每日 Token） | — | — | **无 refresh**（重新导入） |
...
> - **商汤小浣熊不接档位**（上表 `—`）**是刻意的**：实测该上游**默认档才是最深思考**，
>   下发任何档位（含 `high`）反而让思考量锐减约 90%。网关因此**主动剥离** `reasoning_effort`，
>   保证实际走深度思考；代价是无法让该渠道「快速」，需要快就用带 `fast` 标签的模型。
>   （客户端那个「深度思考/快速」开关属于 agent 层参数，本渠道对接的 LLM 网关不接受。）
> - 商汤小浣熊的面板按钮走**浏览器授权**：登录期间把 `office-raccoon://` 协议回调临时指向本工具，
>   接住网页授权码后由本工具完成 token 兑换，结束立即恢复注册表。**登录期间请勿启动小浣熊客户端**
>   （它会重新注册该协议，把本工具的注册覆盖掉）；弹窗里的「从客户端导入」则复用本机已登录客户端的凭据。
>   Loomy / MonkeyCode 为**导入型**渠道，只有导入一条路。
> - MonkeyCode 的积分明细有**两行**：`积分余额`（计入合计）与 `每日 Token 额度`
>   （单位是 token，仅展示、标「仅展示」角标，不计入合计）。两者都取自控制台会话；
>   控制台会话短寿（约 6 天），但导入时会一并读取客户端里的**百智云会话**（约 29 天），
>   失效时自动用它换一份新的 —— 不必为一周就过期的会话反复重导凭据。
> - **小浣熊的两种添加方式会话语义不同**：**浏览器授权登录**走完整授权流程、新建独立会话（新 `sid`），
