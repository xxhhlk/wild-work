...
| R10 | 新增渠道扩展方式：实现 `provider.Upstream` 接口 + auth 加载器 + 注册 Runtime | 模型前缀 `channel/<model>` 路由；已实现 WorkBuddy(国内) + WorkBuddyAI(国际) + TraeWork + Qoder + QoderCN + QoderCOM + 千问办公(qwenwork) 七渠道 |
...
## 5. 渠道（已实现 WorkBuddy 国内版 + WorkBuddyAI 国际版 + TraeWork + Qoder + QoderCN + QoderCOM + 千问办公）
...
> 故 `noExplicitCheckin` 排除 `qoder`（面板不显示手动签到按钮），web/app.js 的
> `NO_EXPLICIT_CHECKIN` 与之同源。QoderCN / QoderCOM 已实现签到（campaigns 主路径），保留手动按钮。
