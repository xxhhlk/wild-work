...
> **性质说明（2026-09-25）**：下列条目除末条外均为**已接受的设计取舍**，非欠账；
> 末条是**待决策项**（非缺陷）。本渠道功能已完整落地。
...
- qoder2api 的 PAT→jobToken 交换未引入（如需粘贴 PAT 登录再评估）。—— **可选增强**
- 稳定运行后是否替换旧 `qoder` 渠道：另行决议（决议前两条渠道并存）。—— **待决策**：
  现状是旧 qoder 已从界面下线、路由保留、不新增功能（AGENTS R10）；此决议由上游作者提出，
  尚未拍板「移除」与否。**不影响当前使用**。
