...
- 为降低抢刷新频率，最初曾把 `raccoon` 移出积分自动刷新循环；**找到余额端点后已加回**（余额要刷）；
  token 另由 4 小时保活维护；
- 并发场景的**根治**已在阶段 E 落地：`internal/provider/refresh.go` 的按账号单飞
  （`RefreshOnce`），实测 8 并发由「1 成功 + 7 conflict + 冷却」变为「8 成功 + 0 conflict」，
  见 `docs/loomy-raccoon阶段E验收报告.md` §4.1；
- 文档明确要求：**导入后退出小浣熊客户端**（单会话 refresh_token 的硬约束，不可绕过）。
...
> **状态更新（2026-09-23，阶段 E 完成后）**：第 1、2 条已由阶段 E 核销；第 3–6 条转入
> `docs/loomy-raccoon阶段E验收报告.md` §5 继续跟踪。详见下方各条标注。

1. ~~`/v1/responses` 与 `/v1/messages`（三接口兼容层）在两渠道上的表现~~ —— ✅ **已核销**（阶段 E §E1）：两渠道三接口均 200，流式事件序列符合各自官方协议；
2. ~~工具调用闭环（两个渠道的 `tool_calls` 流式形状已实现但未端到端验证）~~ —— ✅ **已核销**（阶段 E §E2）：raccoon 默认模型 + loomy 8 模型中 7 个通过（`spark-x` 为模型自身行为）；
3. 小浣熊 refresh 的**真实轮换**验证（本次只在"被抢刷新"场景下观察到冲突，未验证 wild-work 独占时的刷新成功路径）—— ⏳ 阶段 E §5.3 仍列为未验证；
4. 两渠道的 429/限流形态（未触发过）—— ⏳ 阶段 E §5.2 仍列为未验证（单测覆盖分类逻辑，但无真实上游数据）；
5. Web UI 手工验收（导入按钮、文案、配色）—— ⏳ 阶段 E §5.4 仅验证静态资源包含关系，未做浏览器交互；
6. 多账号并发与会话隔离 —— ⏳ 阶段 E §5.5 仅验证跨渠道并发隔离，同渠道多账号未验证。
