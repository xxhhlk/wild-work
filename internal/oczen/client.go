...
type Client struct {
	...
	// （本渠道非流式超时较宽，10 分钟，但同样会截断超过它的流）。空闲兜底改由
	// IdleReader 承担。
...
// New 生产默认客户端。
func New() *Client {
	return &Client{HTTP: &http.Client{Timeout: requestTimeout, Transport: newTransport()}, StreamHTTP: &http.Client{Transport: newTransport()}, Base: DefaultBase}
}
