...
// live_probe_test.go QoderCOM「思考档位」真实账号探针。
//
// 目的：QoderCOM 的档位收口（thinking_config 解析 + parameters.reasoning_effort 投影）
// 是**照着 internal/qoder 移植**的，从未在 QoderCOM 真实上游上验证过。本探针回答三件事：
//
...
//
//	WILDWORK_AUTHDIR=./auths go test -tags live ./internal/qodercom/ -run TestLiveProbe -v -timeout 1800s
//
// 可调环境变量：
//
//	WILDWORK_AUTHDIR       账号目录（默认 ./auths），读取 qodercom-*.json
...
//  3. **档位能力必须先写进 reasoning.Caps 的 RealmQoderCOM 面**，否则 reasoningSpecFor
...
// liveAuth 从 WILDWORK_AUTHDIR（默认 ./auths）取第一个 QoderCOM 账号。
// 注意账号文件名必须是 qodercom-*.json（LoadQoderDir 的 qoder*.json glob 会吞掉它，
...
func liveAuth(t *testing.T) *auth.Auth {
	...
	as, err := auth.LoadQoderCOMDir(abs)
	if err != nil {
		t.Fatalf("加载 %s 下的 qodercom*.json 失败：%v", abs, err)
	}
	if len(as) == 0 {
		t.Skipf("在 %s 没找到 qodercom-*.json —— 用面板登录一个 QoderCOM 账号，"+
			"或把 qoder-*.json 复制一份改名为 qodercom-*.json（两渠道同域 qoder.com.cn，凭据通用）", abs)
	}
	a := as[0]
...
// publishCapsFromEntries 把目录解析出的档位能力写进 RealmQoderCOM 面。
...
func publishCapsFromEntries(entries []ModelEntry) int {
	...
	reasoning.Caps.SetRemote(reasoning.RealmQoderCOM, caps)
	return len(caps)
}
...
func TestLiveProbeModelCatalog(t *testing.T) {
	...
	t.Logf("==> 已把 %d 个模型的能力写入 RealmQoderCOM 面（供后续用例的生产投影使用）", n)
}
...
func TestLiveProbeProductionPath(t *testing.T) {
	...
	if reasoning == "" {
		t.Errorf("生产链路没拿到 reasoning_content —— 该模型 %s 在 QoderCOM 上游不吐思考链，"+
