// Package login_qodercn 实现 QoderCN OAuth 设备授权登录（PKCE Device Flow）。
...
//   - 登录成功后需另行调 /api/v1/userinfo 实测 userType（由 qodercn.Client 填充）
package login_qodercn
...
func Start(client *http.Client, statePath string) (string, error) {
	...
	authURL := fmt.Sprintf("%s/device/selectAccounts?nonce=%s&challenge=%s&challenge_method=S256&client_id=%s",
		OAuthWebsiteCN, nonce, challenge, OAuthClientID)
...
// 文件名 qodercn-<uid>.json（独立前缀，不与 qoder*.json 的 glob 冲突）；
// 机器指纹由调用方在写入前生成并传入。
func SaveAuth(authDir string, r Result, machineID, machineToken, machineType string) (string, error) {
	...
	fp := filepath.Join(authDir, "qodercn-"+r.UID+".json")
	tmp := fp + ".tmp"
