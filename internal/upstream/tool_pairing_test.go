...
// TestCleanupOrphanThroughPrepareBody 集成：孤儿 tool_call 经 PrepareBody 全链路被剔除，
// 且与消息顺序无关。
func TestCleanupOrphanThroughPrepareBody(t *testing.T) {
	...
	if _, ok := asst["tool_calls"]; ok {
		t.Fatalf("orphan tool_calls should be stripped by PrepareBody, got %#v", asst)
	}
}
...
// 配对对称。走的是 PrepareBody 生产转换路径，而非只调内部函数。
